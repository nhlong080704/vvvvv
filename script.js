...
scene.fog = new THREE.FogExp2(0x000000, 0.002); // Tăng fog từ 0.0015 lên 0.002 để che bớt đối tượng xa

const camera = new THREE.PerspectiveCamera(75, window.innerWidth / window.innerHeight, 0.1, 100000);
camera.position.set(0, 20, 30);

const renderer = new THREE.WebGLRenderer({ antialias: false }); // Tắt antialias để tăng hiệu năng
renderer.setSize(window.innerWidth, window.innerHeight);
renderer.setPixelRatio(Math.min(window.devicePixelRatio, 1.5)); // Giới hạn pixel ratio
...
for (let i = 0; i < 4; i++) { // 📍 CHỖ SỬA 5: Giảm thêm từ 8 xuống 4
  const hue = Math.random() * 360;
  const color = `hsla(${hue}, 80%, 50%, 0.6)`;
  const nebula = createGlowMaterial(color, 128); // Giảm kích thước từ 256 xuống 128
  nebula.scale.set(80, 80, 1); // Giảm kích thước từ 100 xuống 80
  nebula.position.set(
    (Math.random() - 0.5) * 175,
    (Math.random() - 0.5) * 175,
    (Math.random() - 0.5) * 175
...
const galaxyParameters = {
  count: 25000, // 📍 CHỖ SỬA 1: Giảm thêm từ 50000 xuống 25000
  arms: 6,
  radius: 100,
  spin: 0.5,
  randomness: 0.2,
...
const defaultHeartImages = [
  'https://raw.githubusercontent.com/nhlong080704/vvvvv/main/images/img1.png',
  'https://raw.githubusercontent.com/nhlong080704/vvvvv/main/images/img2.png', 
  'https://raw.githubusercontent.com/nhlong080704/vvvvv/main/images/img3.png',
...
const maxDensity = 12000; // 📍 CHỖ SỬA 2: Giảm thêm từ 25000 xuống 12000
// Mật độ điểm khi có 10 ảnh trở lên (thấp nhất)
const minDensity = 500; // 📍 CHỖ SỬA 3: Giảm thêm từ 1000 xuống 500
...
function createNeonTexture(image, size) {
  ...
  const cornerRadius = size * 0.05; // Giảm từ 0.1 xuống 0.05 để đơn giản hóa
  ctx.save();
  ctx.beginPath();
...
for (let group = 0; group < numGroups; group++) {
  ...
  img.onload = () => {
    const neonTexture = createNeonTexture(img, 128); // Giảm kích thước texture từ 256 xuống 128

    // Material khi ở gần
    const materialNear = new THREE.PointsMaterial({
      size: 1.5, // Giảm kích thước từ 1.8 xuống 1.5
      map: neonTexture,
      transparent: false,
      alphaTest: 0.2,
      depthWrite: true,
...
const starCount = 5000; // 📍 CHỖ SỬA 4: Giảm thêm từ 10000 xuống 5000
...
const starMaterial = new THREE.PointsMaterial({
  color: 0xffffff,
  size: 0.5, // Giảm kích thước từ 0.7 xuống 0.5
  transparent: true,
  opacity: 0.6, // Giảm độ mờ từ 0.7 xuống 0.6
  depthWrite: false
});
...
function createShootingStar() {
  ...
  // Đầu sao băng
  const headGeometry = new THREE.SphereGeometry(2, 16, 16); // Giảm độ phức tạp từ 32,32 xuống 16,16
  ...
  const glowGeometry = new THREE.SphereGeometry(3, 16, 16); // Giảm độ phức tạp từ 32,32 xuống 16,16
  ...
  const atmosphereGeometry = new THREE.SphereGeometry(planetRadius * 1.05, 24, 24); // Giảm từ 48,48 xuống 24,24
...
function createPlanetTexture(size = 256) { // Giảm kích thước từ 512 xuống 256
  const canvas = document.createElement('canvas');
  canvas.width = canvas.height = size;
  const ctx = canvas.getContext('2d');
  ...
  // Các đốm màu ngẫu nhiên - giảm số lượng
  const spotColors = ['#f8bbd0', '#f48fb1', '#f06292', '#ffffff', '#e1aaff', '#a259f7'];
  for (let i = 0; i < 20; i++) { // Giảm từ 40 xuống 20
    const x = Math.random() * size;
    const y = Math.random() * size;
    const radius = 20 + Math.random() * 80; // Giảm kích thước
  ...
  // Các đường cong (swirls) - giảm số lượng
  for (let i = 0; i < 4; i++) { // Giảm từ 8 xuống 4
    ...
    ctx.lineWidth = 6 + Math.random() * 12; // Giảm độ dày
    ctx.stroke();
  }
...
const planetGeometry = new THREE.SphereGeometry(planetRadius, 32, 32); // Giảm từ 48,48 xuống 32,32
...
function createTextRings() {
  ...
  for (let i = 0; i < numRings; i++) {
    ...
    const ringGeometry = new THREE.CylinderGeometry(ringRadius, ringRadius, 1, 64, 1, true); // Giảm từ 128 xuống 64
...
let frameCount = 0; // Thêm bộ đếm frame để tối ưu hiệu năng
...
function animate() {
  ...
  if (!introStarted) {
    ...
    fadeOpacity = 0.1;
    // Chỉ cập nhật scene traverse mỗi 5 frame để tối ưu hiệu năng
    if (frameCount % 5 === 0) {
      scene.traverse(obj => {
    ...
    // Chỉ cập nhật scene traverse mỗi 3 frame để tối ưu hiệu năng
    if (frameCount % 3 === 0) {
      scene.traverse(obj => {
  ...
  if (shootingStars.length < 2 && Math.random() < 0.015) { // Giảm số lượng từ 3 xuống 2 và tần suất từ 0.02 xuống 0.015
    createShootingStar();
  }

  // Logic chuyển đổi material cho các nhóm điểm trái tim - chỉ cập nhật mỗi 10 frame
  if (frameCount % 10 === 0) {
    scene.traverse(obj => {
      if (obj.isPoints && obj.userData.materialNear && obj.userData.materialFar) {
        const positionAttr = obj.geometry.getAttribute('position');
        let isClose = false;
        // Chỉ kiểm tra một số điểm để tối ưu hiệu năng
        const checkInterval = Math.max(1, Math.floor(positionAttr.count / 100));
        for (let i = 0; i < positionAttr.count; i += checkInterval) {
...
if (starField && starField.geometry) {
  starField.geometry.setDrawRange(0, Math.floor(originalStarCount * 0.05)); // Giảm từ 0.1 xuống 0.05
}
